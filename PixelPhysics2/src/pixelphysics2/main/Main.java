...
import pixelphysics2.ui.InitView;
...
public class Main {
	...
	public static final boolean MACOS = true;
	...
	public static void resetParticles() {
		...
		for (int i = 0; i < particleNum; i++) {
			...
			if(LWJGL){
				x = (int)(Math.random() * (double)OpenGL.WIDTH);
				y = (int)(Math.random() * (double)OpenGL.HEIGHT);
			}
			...
			// Data.particles[i].vX = (Math.random() -0.5) * 40;
			// Data.particles[i].vY = (Math.random() -0.5) * 40;
			if (i > 0) {
	...
	public static void update() {
		...
		if (LWJGL) {
			...
			VolatileImage vImage2 = gc.createCompatibleVolatileImage(vImage.getWidth(), vImage.getHeight());
			Graphics2D g2v = (Graphics2D) vImage2.getGraphics();
			Drawing.doBackGroundDraw(g2v);
			Drawing.doDraw(g2v);
			g2b.drawImage(vImage2, 0, 0, null);
			vImage.flush();
		}
	}

	public static boolean isRMBClicked() {
		return Inputerface.RMB;
	}
