...
public class Main {
	...
	public static void resetParticles() {
		...
		for (int i = 0; i < particleNum; i++) {
			...
			Data.particles[i].RGB = rand.nextInt(255) << 24 | rand.nextInt(255) << 16 | rand.nextInt(255) << 8
					| rand.nextInt(255);
	...
	public static void calculateLastMouseRGB() {
	...
	public static void doDataDraw(Graphics2D g2b, VolatileImage retVal) {
		g2b.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
	...
	public static void doPulls() {
		if (Inputerface.keySet[KeyEvent.VK_CONTROL]) {
			Point pt = new Point();
			pt.setLocation(Data.lastMouse.x, Data.lastMouse.y);
			Data.pausedPullArray.add(pt);
		} else if(Inputerface.rightClick){
			int pX = Data.lastMouse.x;
			int pY = Data.lastMouse.y;
			while(true){
				for (int j = 0; j < Data.particles.length; j++) {
					Particle p = Data.particles[j];
					double dist = getDistance(p.x, p.y, pX, pY);
					double deltaX = (p.x - pX) / dist;
					double deltaY = (p.y - pY) / dist;
					p.vX -= deltaX * Data.forceMult / 5D;
					p.vY -= deltaY * Data.forceMult / 5D;
				}
				if(Data.pausedPullArray.size() != 0) {
					Point pt = Data.pausedPullArray.get(Data.pausedPullArray.size()-1);
					pX = pt.x;
					pY = pt.y;
					Data.pausedPullArray.remove(pt);
				}
				else break;
			};
	...
	public static void doDraw(Graphics2D g2v, Graphics2D g2b) {
		for (int i = 0; i < particleNum; i++) {
			...
			// TODO Fix this flag statement with something streamline, and switch this
			// Data.s to a switch-case thing
